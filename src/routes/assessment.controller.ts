...
const getAssessmentsByAssetId = async (req: UserRequest, res: Response) => {
    ...
    const assessment = await getConnection().getRepository(Assessment).find({
...
const getAssessmentVulns = async (req: UserRequest, res: Response) => {
    ...
    const vulnerabilities = await getConnection().getRepository(Vulnerability).find({
...
const updateAssessmentById = async (req: UserRequest, res: Response) => {
    ...
    let assessment = await getConnection().getRepository(Assessment).findOne(req.params.assessmentId);
    if (!assessment) {
        return res.status(404).json('Assessment does not exist');
    }
...
const queryReportDataByAssessment = async (req: UserRequest, res: Response) => {
    ...
    const assessment = await getConnection().getRepository(Assessment).findOne(req.params.assessmentId, { relations: ['asset'] });
    const asset = await getConnection().getRepository(Asset).findOne(assessment.asset.id, {
        relations: ['organization']
    });
    const organization = await getConnection().getRepository(Organization).findOne(asset.organization.id);
    const vulnerabilities = await getConnection().getRepository(Vulnerability)
